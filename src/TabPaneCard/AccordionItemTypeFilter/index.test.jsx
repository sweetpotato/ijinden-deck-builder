...
test('デフォルトのレンダリング', () => {
  ...
  // ヘッダ部分はボタンとして取得できる
  expect(getByRole('button', { name: /種類とパワー/, expanded: false })).toBeVisible()
})
...
describe('チェックボックスの選択', () => {
  ...
  describe('最後は「すべて」を選択してリセットする', () => {
    ...
    test('イジンとマリョクを選択する', async () => {
      ...
      expect(getType(result)).toBe(enumType.IJIN | enumType.MARYOKU)
      expect(getPower(result)).toBe(0)
      expect(getComparator(result)).toBe(enumComparator.GE)
      expect(getByRole('checkbox', { name: 'すべて' })).not.toBeChecked()
      expect(getByRole('checkbox', { name: 'イジン' })).toBeChecked()
